    ...
    <v-card>
      ...
      <v-list two-line class="orange lighten-2">
        <template v-for="(item, index) in items" >
            <v-subheader v-if="item.header" :key="item.header" class="white--text">{{ item.header }}</v-subheader>
    ...
    return {
      items: [{
        ...
        },
        {
          ...
          subtitle: "<span class='text--primary'>cc: Daniel</span> &mdash; Tell me more..."
        },
        {
          divider: true,
          inset: true
